...
(define (ripple-carry-adder wire-lstA wire-lstB wire-lstS wireCN)
  ;=================
  (define (rca-iter wiresA wiresB wiresS wireCin); wireCout)
    ;!需要增加停止的条件
    (if (null? (cdr wiresA))
        (let ((tpwireA (car wiresA))
               (tpwireB (car wiresB))
               (tpwireS (car wiresS)))
          (lastFullAdder tpwireA tpwireB wireCin tpwireS wireCN)
           )
         (let ((tpwireA (car wiresA))
               (tpwireB (car wiresB))
               (tpwireS (car wiresS)))
           (let ((wireCout (make-wire)))
             (full-adder tpwireA tpwireB wireCin tpwireS wireCout)
             (rca-iter (cdr wiresA) (cdr wiresB) (cdr wiresS) wireCout)
             )
           )
    )
    )
  (define (firstFullAdder wireA wireB wireS)
    ;
    (let ((wireCout (make-wire))
          (wireC (make-wire)))
      (full-adder wireA wireB wireC wireS wireCout)
      wireCout
      )
    )
  (define (lastFullAdder wireAn wireBn wireCin wireSn wireCn)
    (full-adder wireAn wireBn wireCin wireSn wireCn))
  ;进入真正的运算
  (if (null? (cdr wire-lstA))
      ;如果只有一个元素
      (let ((tpwireA (car wire-lstA))
            (tpwireB (car wire-lstB))
            (tpwireS (car wire-lstS))
            (wireCin (make-wire)))
        (lastFullAdder tpwireA tpwireB wireCin tpwireS wireCN)
        )
      (let ((tpwireA (car wire-lstA))
            (tpwireB (car wire-lstB))
            (tpwireS (car wire-lstS)))
        (let ((wireCout (firstFullAdder tpwireA tpwireB tpwireS)))
          (rca-iter (cdr wire-lstA) (cdr wire-lstB) (cdr wire-lstS) wireCout)
          )
        )
      )
  )
