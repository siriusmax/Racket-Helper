...
(define (analyze exp) ;返回值是一个可执行函数(不是源代码形式的也不是函数对象),以环境为参数。
 (cond ((self-evaluating? exp)  (analyze-self-evaluating exp))
       ...
       ;TODO try-again写在里面还是外面？ 这是一个输入命令，感觉写在外面更好些
...
(define (analyze-all-answer exp)  ; exp形如  (all-answer (amb 1 2 4))
  (let ((proc (analyze (cadr exp))))  ; 这意味着analyze 后面只能有一条表达式
    (lambda (env succeed fail)
      (proc env
            ...
            (lambda ()
              ;(displayln "self-defined fail in all-answer called!")
              (void)
              (driver-loop) ;重新启动流程
              )
            )
...
(define (get-if-fail-do-clause exp)
  (cadr exp))
(define (get-if-fail-fail-clause exp)
  (caddr exp))
(define (analyze-if-fail exp)  ; 最后返回的形式 分派函数
  ;(analyze )  
  (let ((doproc (analyze (get-if-fail-do-clause exp)))
        (failproc (analyze (get-if-fail-fail-clause exp))))
    (lambda (env succeed fail)
      (doproc env
              (lambda (val fail2)
                (doproc env succeed fail))  ;TODO  为什么这里一定要用这一句，换成直接返回val就不行呢？  还是不理解调用的路径
                ; val))
              (lambda () ;调用fail的时候是没有参数的
                (failproc env succeed fail))
              )
      )
    )
  )
...
;====测试语句
#|
(define testdef '(let ((m2 (if-fail 4 50)))
  (require (> m2 20))
  m2)
)
...
(define testdef2 '(all-answer (amb 1 2 4)))
...
(define (driver-loop)
  ...
  (internal-loop
   (lambda ()
      (display "There are no more answers.")(newline)  ;TODO 按照题目中的定义，应该求值到没有解但是改问题还没有结束的状态，但如果问题没有结束，又如何知道这个问题已经没有解了呢？
     (driver-loop))))
(driver-loop)
