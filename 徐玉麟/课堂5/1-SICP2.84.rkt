...
; 在此处补充你的代码
  (define (raise-rational rat)
    (let((ratcontent (contents rat)))
      (make-real (/ (car ratcontent) (cdr ratcontent)))))
  (define (raise-real r)
  ...
  (put-coercion 'integer 'real
                (lambda (x) (raise-rational(raise-integer x))))
  (put-coercion 'integer 'complex
                (lambda (x) (raise-real(raise-rational(raise-integer x)))))
  (put-coercion 'rational 'real raise-rational)
  (put-coercion 'rational 'complex
                (lambda (x) (raise-real(raise-rational x))))
    ...
    (let ((proc (get op type-tags)))
      (if proc
          ...
          (if (= (length args) 2)
              ...
              (error "No method for these types"
                     (list op type-tags)))))))
(define (content->list cnt)
  (if(pair? cnt)
     (list (car cnt) (cdr cnt))
     (list cnt)))
(define (display-obj obj)
  (let((tag (type-tag obj))
       (content (contents obj)))
    (if(eq? tag 'complex)
       (set! content (cdr content))
       (void))
    (displayln (cons tag (content->list content)))))
; 以上是补充的代码
