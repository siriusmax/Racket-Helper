...
;在此处补充你的代码
  (define (add0 Ai Bi Si Ci)
    (let ((Ci+1 (make-wire)))
      (if (null? (cdr Ai))
          (full-adder (car Ai) (car Bi) Ci (car Si) wireCN)
          (begin
            (full-adder (car Ai) (car Bi) Ci (car Si) Ci+1)
            (add0 (cdr Ai) (cdr Bi) (cdr Si) Ci+1)))))
  (add0 wire-lstA wire-lstB wire-lstS (make-wire)))
  ;以上为补充的代码
