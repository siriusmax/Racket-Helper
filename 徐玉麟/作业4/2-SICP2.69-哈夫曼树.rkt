...
(define (generate-huffman-tree pairs)
  (define leafset (make-leaf-set pairs))
  (define (genhuff0 nodeset)
    (if(null? (cdr nodeset))
       (car nodeset)
       (let((min1 (car nodeset))
            (min2 (cadr nodeset)))
         (genhuff0 (adjoin-set (make-code-tree min2 min1) (cddr nodeset))))))
  (genhuff0 leafset))

(define (contains symbol symbollist)
   (accumulate myor #f (map (lambda (x) (equal? (symbol->string x) (string symbol))) symbollist)))

(define (get-next-bit symbol branch)
  (if(contains symbol (symbols (left-branch branch)))
     0
     1))

(define (get-bits symbol tree)
  (define (get-bits0 symbol branch bits)
    (if(leaf? branch)
       bits
       (let((nextbit (get-next-bit symbol branch)))
         (if(= 0 nextbit)
            (get-bits0 symbol (left-branch branch) (append bits (list 0)))
            (get-bits0 symbol (right-branch branch) (append bits (list 1)))))))
  (get-bits0 symbol tree '()))         
  
(define (encode str tree)
  (define charlist (string->list (symbol->string str)))
  (accumulate append '()
              (map (lambda (char) (get-bits char tree)) charlist)))
