...
(define (apply-generic op . args)
  (define (get-type-priority type)
    (cond ((eq? type 'integer) 1)
          ...
          ((eq? type 'real) 3)
          ((eq? type 'complex) 4)))
  (define (get-type-cast-function nowTypePriority)
    (cond ((= nowTypePriority 1) (get 'raise 'integer))
          ((= nowTypePriority 2) (get 'raise 'rational))
          ((= nowTypePriority 3) (get 'raise 'real))))
      ...
      (if proc
          ...
          (if (= (length args) 2)
              (let ((type1 (car type-tags))
                    ...
                    (a1 (car args))
                    (a2 (cadr args)))
                (let ((priority1 (get-type-priority type1))
                      (priority2 (get-type-priority type2)))
                  (if (< priority1 priority2)
                      (apply-generic op ((get-type-cast-function priority1) a1) a2)
                      (apply-generic op a1 ((get-type-cast-function priority2) a2)))))
              (error "no such op"))))))
