...
(define (special-clauses? clause)
  (let ((signal (cdr clause)))
    (if (null? signal)
        false
        (eq? (car signal) '=>))))
(define (special-cond-predicate clause) (car clause))
(define (special-cond-actions clause) (caddr clause))
        ...
        (if (cond-else-clause? first)
            (if (null? rest)
                ...
                (error "ELSE clause isn't last -- COND->IF"
                       clauses))
            (if (special-clauses? first)
                (make-if (special-cond-predicate first)
                         (list (special-cond-actions first) (special-cond-predicate first))
                         (expand-clauses rest))
