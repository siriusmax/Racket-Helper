...
(define (force-it obj)
  (cond ((thunk? obj) (actual-value (thunk-exp obj) (thunk-env obj)))
        ((thunk-memo? obj)
         (let ((result (actual-value (mcar (mcdr obj)) (mcar (mcdr (mcdr obj))))))
           (set-mcar! obj 'thunk-memo-evaluated)
           (set-mcar! (mcdr obj) result)
           (set-mcdr! (mcdr obj) '())
           result))
        ((thunk-memo-evaluated? obj) (thunk-value obj))
        (else obj)))
...
(define (thunk-memo? obj)
  (if (mpair? obj)
      (equal? (mcar obj) 'thunk-memo)
      false))
(define (thunk-memo-evaluated? obj)
  (if (mpair? obj)
      (equal? (mcar obj) 'thunk-memo-evaluated)
      false))
...
(define (my-apply procedure arguments env)
  (cond ((primitive-procedure? procedure)
         (apply (primitive-implementation procedure) (list-of-arg-values arguments env)))
        ...
        ((compound-procedure? procedure) (eval-sequence
                                          ...
                                          (extend-environment
                                           ...
                                           (list-of-different-args (procedure-parameters procedure) arguments env)
...
(define (lazy? operand)
  (equal? (cadr operand) 'lazy))
(define (lazy-memo? operand)
  (equal? (cadr operand) 'lazy-memo))
(define (list-of-different-args param-names exps env)
  (if (no-operands? exps)
      '()
      (let ((now-param-name (car param-names)))
        (if (pair? now-param-name)
            (cond ((lazy? now-param-name)
                   (mcons (delay-it (first-operand exps) env)
                          (list-of-different-args (cdr param-names) (rest-operands exps) env)))
                  ((lazy-memo? now-param-name)
                   (mcons (delay-memo-it (first-operand exps) env)
                          (list-of-different-args (cdr param-names) (rest-operands exps) env)))
                  (else
                   (error "unknown parameter modifier")))
...
(define (user-print object)
  (cond ((equal? (void) object) (void))
        ((equal? 'ok object) (void))
