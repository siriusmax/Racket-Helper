...
(define (make-swap inst machine pc)
  (let ((first-name (cadr inst))
        (second-name (caddr inst)))
    (let ((first-register (get-register machine first-name))
          (second-register (get-register machine second-name)))
      (lambda ()
        (define temp (get-contents first-register))
        (set-contents! first-register (get-contents second-register))
        (set-contents! second-register temp)
        (advance-pc pc)))))
...
(define (run-machine machine input output) 
  ...
  (if label-has-failed
      (displayln "label error in machine")
      (begin
        (start machine)
        (output-machine machine output)
        (newline))))

(define label-has-failed #f)
(define (process-loop)
            ...
            (if (eq? (car m-txt) 'make-machine)
                ...
                (if label-has-failed
                    (begin
                      ...
                      (inner-loop))
                    (begin
                      (run-machine machine m-txt (read)) 
                      (inner-loop)))))))
    (inner-loop)))
